<section class="bg-[#dbb186] border rounded-3xl border-transparent">
    ...
    <div class="mx-[20px] md:mx-[100px]">
        <div class="overflow-x-auto mb-4">
            <table class="table-auto w-full text-left align-middle">
                <thead class="bg-[#CE9568]">
                    <tr>
                        ...
                        <th scope="col" wire:click="ordenar('description')"
                            class="min-w-[150px] max-w-[300px] whitespace-nowrap overflow-hidden text-ellipsis pb-2 cursor-pointer">
                            @if ($campoOrden == 'description' && $orden)
                                @if ($orden == 'asc')
                                    <p class="text-[#E50000]">Descripcion ⬆</p>
                                @else
                                    <p class="text-[#E50000]">Descripcion ⬇</p>
                                @endif
                            @else
                        ...
                        <th scope="col" wire:click="ordenar('country_id')"
                            class="min-w-[150px] max-w-[300px] whitespace-nowrap overflow-hidden text-ellipsis pb-2 cursor-pointer">
                            @if ($campoOrden == 'country_id' && $orden)
                                @if ($orden == 'asc')
                                    <p class="text-[#E50000]">Pais ⬆</p>
                                @else
                                    <p class="text-[#E50000]">Pais ⬇</p>
                                @endif
                            @else
                ...
                <tbody class="list">
                    @forelse  ($allDestination as $Destination)
                        <tr class="m-2">
                            ...
                            <td class="min-w-[150px] max-w-[300px] whitespace-nowrap overflow-hidden text-ellipsis">
                                @foreach ($allCountries as $country)
                                    @if ($country->id == $Destination->country_id)
                                        {{ $country->name }}
                                    @endif
                                @endforeach
                            ...
                            <td
                                ...
                                <button class="BtnDetalles m-2 min-w-[90px]">
                                    ...
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                        ...
                                        onclick="showDestinationsDetails({{ $Destination->id }})">
                                ...
                                @if (Auth::user()->range_id >= 11)
                                    ...
                                    <form action="/deleteDestination/{{ $Destination->id }}" method="delete"
                                        class="m-1">
                                        @csrf
                                        @method('delete')
            ...
            @if ($allDestination->hasPages())
                <div>
                    {{ $allDestination->links() }}
                </div>
            @endif
        </div>
    </div>
